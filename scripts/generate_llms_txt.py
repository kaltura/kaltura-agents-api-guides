...
def main():
    ...
    output.append("")
    output.append(
        "> Live-tested API guides for the Kaltura video platform. "
        "Covers authentication, upload, search, player embed, AI services, "
        "virtual events, and multi-stream — all with curl examples for "
        "AI agents and developers."
    )
    output.append("")
    output.append(
        "Kaltura is a Video Platform as a Service (VPaaS) with 80+ REST API "
        "services. These guides document the most commonly needed APIs with "
        "real, tested curl examples using shell variables. Every guide has "
        "companion test scripts validated against the live Kaltura API."
    )
    output.append("")
