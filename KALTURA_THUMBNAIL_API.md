...
**Base URL:** `https://www.kaltura.com/api_v3` (thumbAsset/thumbParams services); `https://cdnapisec.kaltura.com` (dynamic thumbnail URLs)  
**Auth:** thumbAsset/thumbParams use `-d "ks=$KALTURA_KS"` form param. Dynamic thumbnail URLs work without KS for public entries; access-controlled entries require `/ks/{KS}` appended to the URL.  
**Format:** Form-encoded request body; JSON response (format=1)  

<!-- Sections: 1.Prerequisites | 2.When to Use | 3.Dynamic Thumbnail API (URL-Based) | 4.thumbAsset API (Stored Thumbnails) | 5.thumbParams API (Thumbnail Templates) | 6.When to Use Which | 7.Error Handling | 8.Best Practices | 9.Related Guides -->


# 1. Prerequisites

- **Kaltura account** with a Partner ID (`$KALTURA_PARTNER_ID`)  
- **Admin KS** (`$KALTURA_KS`) for thumbAsset and thumbParams API calls (generate via `session.start` or AppToken flow — see [Session Guide](KALTURA_SESSION_GUIDE.md))  
- **Entry ID** (`$KALTURA_ENTRY_ID`) of a media entry in READY status (status=2) for frame capture operations  
- **Service URL** (`$KALTURA_SERVICE_URL`) — defaults to `https://www.kaltura.com/api_v3`  
- Dynamic thumbnail URLs require only the Partner ID and Entry ID; KS is needed only for access-controlled entries  
...
# 3. Dynamic Thumbnail API (URL-Based)
...
## 3.4 Source Crop (Region of Interest)
...
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/640/height/360/type/3/vid_sec/30

# High-quality PNG for export
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/1920/height/1080/quality/100/format/png

# CSS sprite strip: 10 slices for hover preview
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/160/height/90/vid_slices/10

# Padded with white background (letterboxed)
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/400/height/400/type/2/bgcolor/ffffff

# Sprite strip from specific time range (10s to 60s)
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/160/height/90/vid_slices/10/start_sec/10/end_sec/60

# Individual slice from a sprite (slice 3 of 10)
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/160/height/90/vid_slices/10/vid_slice/3

# Region-of-interest crop
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/src_x/100/src_y/50/src_w/800/src_h/450/width/400/height/225

# Social media share card (1200x630, padded)
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/1200/height/630/type/2/bgcolor/000000/quality/90

# Force exact dimensions (stretch)
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/300/height/300/type/5

# Download as file
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/1920/height/1080/file_name/thumbnail.jpg

# Access-controlled entry
https://cdnapisec.kaltura.com/p/$KALTURA_PARTNER_ID/thumbnail/entry_id/$KALTURA_ENTRY_ID/width/640/height/360/ks/$KALTURA_KS
```

## 3.7 Behavior Notes

- **KS is optional** for public entries. The endpoint auto-detects the partner from the entry ID.  
- **Image entries** (mediaType=2): The original uploaded image is used as the thumbnail source. `vid_sec`, `vid_slice`, and `vid_slices` apply only to video entries.  
...
- **`vid_sec` capping**: Values beyond the entry duration are clamped to the last second gracefully.  
...
# 4. thumbAsset API (Stored Thumbnails)
...
```bash
curl -X POST "$KALTURA_SERVICE_URL/service/thumbAsset/action/addFromImage" \
  -F "ks=$KALTURA_KS" \
  -F "format=1" \
...
curl -X POST "$KALTURA_SERVICE_URL/service/thumbAsset/action/generateByEntryId" \
  -d "ks=$KALTURA_KS" \
  -d "format=1" \
  -d "entryId=$KALTURA_ENTRY_ID" \
...
## 4.5 List Thumbnails for an Entry
...
To delete a thumb asset that is currently the default, first set a different thumb asset as default using `thumbAsset.setAsDefault`.

## 4.7 Serve with On-the-Fly Resize

```bash
# Serve with specific dimensions
curl -X POST "$KALTURA_SERVICE_URL/service/thumbAsset/action/serve" \
  -d "ks=$KALTURA_KS" \
  -d "format=1" \
  -d "thumbAssetId=$THUMB_ASSET_ID" \
  -d "thumbParams[objectType]=KalturaThumbParams" \
...
# 5. thumbParams API (Thumbnail Templates)
...
| `THUMB_ASSET_IS_DEFAULT` | Attempted to delete the current default thumb asset | Set a different thumb as default first, then delete |
...
- **Use the dynamic thumbnail URL for display.** The URL API serves images at any size with CDN caching — stored thumb assets are for editorial purposes (custom uploads, saved captures).  
...
- **Set `vid_sec` for consistent thumbnails.** Specifying a timestamp (e.g., `/vid_sec/5`) ensures every entry in a gallery uses the same relative frame position, producing a uniform visual grid.  
...
- **Keep sprite dimensions within pixel limits.** Ensure `width × vid_slices < 65,500` and `height × vid_slices < 65,500`.  
- **Upload custom thumbnails for editorial control.** For featured content, upload purpose-designed thumbnails via `thumbAsset.addFromImage` to ensure the ideal visual representation.  
